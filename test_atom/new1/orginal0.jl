...
sub_from_b = sub_system(sys_b, [var_a, var_b])
...
roAwB = multiply_star(map((x) -> smallLikeZero(x),pinv(sub_from_b.distribution)), roAB_test)
roAwB = roAwB / tr(roAwB)
...
sys_a_b = DiscreteQuantumSystem([var_b], [var_a], roAwB);
